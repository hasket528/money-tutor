...
// ─── IndexedDB 鍵值儲存層（對照 localStorage 版本）──────────────
// 用一個 key-value object store 取代 localStorage，介面盡量貼近 getItem/setItem。
// 好處：容量大（不受 5MB 限制）、可直接存陣列/物件（結構化複製，免 JSON.stringify）。
const IDB = (() => {
    const DB_NAME = 'rewardSystemDB';
    const DB_VERSION = 1;
    const STORE = 'kv';
    let _db = null;
    function open() {
    ...
    async function setMany(obj) {
        ...
        return new Promise((resolve, reject) => {
            ...
            Object.entries(obj).forEach(([k, v]) => store.put({ k, v }));
    ...
    return { get, setMany, clearAll };
})();

// 更新頁面上的儲存方式徽章（顯示實際已用/可用容量，方便對照差異）
async function updateStorageBadge() {
    const badge = document.getElementById('idbBadge');
    if (!badge) return;
    let extra = '';
    try {
        if (navigator.storage && navigator.storage.estimate) {
            ...
            extra = `｜本網站已用 ${mb(usage)} MB / 可用約 ${mb(quota)} MB`;
        }
    } catch (e) { /* 部分瀏覽器不支援，略過 */ }
    badge.textContent = `💾 儲存方式：IndexedDB（容量大，不受 5MB 限制）${extra}`;
}
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // --- IndexedDB 持久化功能（取代 localStorage）---
    // 注意：pendingRewards（跨頁面通訊）與 rewardSystemZoom（顯示偏好）仍留在 localStorage，
    //      因為各教學單元是把待發放獎勵寫進 localStorage，必須維持同一管道。
    const STORAGE_KEY = 'rewardSystemStudents';

    // 同名保留 saveToLocalStorage，避免改動十多處呼叫點；實際寫入 IndexedDB。
    function saveToLocalStorage() {
        IDB.setMany({
            rewardSystemStudents: students,                       // 直接存陣列（結構化複製）
            rewardSystemDisplayed: [...displayedStudentIds],
            rewardSystemIcons: uploadedIcons,
            rewardSystemSelectedIcon: bonusIconSelect.value,
            rewardSystemSounds: uploadedSounds,
        }).then(() => {
            console.log('💾 學生資料已儲存到 IndexedDB');
            updateStorageBadge();
        }).catch((e) => {
            console.error('儲存到 IndexedDB 失敗:', e);
            alert('儲存失敗：' + (e && e.message ? e.message : e));
        });
    }

    // 一次性遷移：若 IndexedDB 尚無資料、但舊版 localStorage 有，沿用過來（原 localStorage 不刪，方便對照）。
    async function maybeMigrateFromLocalStorage() {
        const already = await IDB.get('rewardSystemStudents');
        if (already) return;                                      // IndexedDB 已有資料，不遷移
        const lsStudents = localStorage.getItem('rewardSystemStudents');
        if (!lsStudents) return;                                  // 舊版也沒有資料
        try {
            const parse = (k, fb) => { const v = localStorage.getItem(k); return v ? JSON.parse(v) : fb; };
            const migStudents = JSON.parse(lsStudents);
            await IDB.setMany({
                rewardSystemStudents: migStudents,
                rewardSystemDisplayed: parse('rewardSystemDisplayed', migStudents.map(s => String(s.id))),
                rewardSystemIcons: parse('rewardSystemIcons', []),
                rewardSystemSounds: parse('rewardSystemSounds', []),
                rewardSystemSelectedIcon: localStorage.getItem('rewardSystemSelectedIcon') || '',
            });
            console.log('🔄 已將舊版 localStorage 資料遷移到 IndexedDB（原 localStorage 保留不動）');
        } catch (e) { console.warn('遷移失敗，改用空白起始:', e); }
    }
    ...
    async function loadFromLocalStorage() {
        try {
            ...
            const savedStudents = await IDB.get('rewardSystemStudents');
            if (savedStudents) {
                students = savedStudents;
                console.log(`📂 已從 IndexedDB 載入 ${students.length} 位學生資料`);
            }
            // 載入顯示中學生 ID（向後相容：若無記錄則顯示所有學生）
            const savedDisplayed = await IDB.get('rewardSystemDisplayed');
            if (savedDisplayed) {
                displayedStudentIds = new Set(savedDisplayed.map(String));
            } else {
                students.forEach(s => displayedStudentIds.add(String(s.id)));
            }
            // 載入自訂加分圖示並重建下拉選單
            const savedIcons = await IDB.get('rewardSystemIcons');
            if (savedIcons) {
                uploadedIcons = savedIcons;
                ...
                console.log(`📂 已從 IndexedDB 載入 ${uploadedIcons.length} 個自訂圖示`);
            }
            // 還原上次選取的加分圖示
            const savedSelectedIcon = await IDB.get('rewardSystemSelectedIcon');
            ...
            // 載入自訂加分音效並重建下拉選單
            const savedSounds = await IDB.get('rewardSystemSounds');
            if (savedSounds) {
                uploadedSounds = savedSounds;
                ...
                console.log(`📂 已從 IndexedDB 載入 ${uploadedSounds.length} 個自訂音效`);
            }
    ...
    if (iconDeleteBtn) {
        iconDeleteBtn.addEventListener('click', () => {
            ...
            if (confirm(`確定要刪除「${iconName}」嗎？`)) {
                ...
                updateIconDeleteBtnVisibility();
                saveToLocalStorage();   // 同時持久化目前選取的圖示到 IndexedDB
                refreshDisplay(!!document.querySelector('.rank-number'));
            }
        });
    }
    ...
    bonusIconSelect.addEventListener('change', () => {
        ...
        updateIconDeleteBtnVisibility();
        saveToLocalStorage();   // 持久化目前選取的圖示到 IndexedDB
        refreshDisplay(!!document.querySelector('.rank-number'));
    });
    ...
    if (clearAllButton) {
        clearAllButton.addEventListener('click', () => {
            if (confirm('確定要清除所有資料嗎？包含學生照片、分數、自訂加分圖示與自訂加分音效，此操作無法復原。')) {
                ...
                // 清除 IndexedDB 內所有資料；pendingRewards 仍在 localStorage，一併移除
                IDB.clearAll().then(() => updateStorageBadge());
                localStorage.removeItem('pendingRewards');
    ...
    // Trigger a display refresh with ranks enabled
     ...
     // 從 IndexedDB 載入學生資料（非同步，需等待後再渲染）
